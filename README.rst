##################################
Building and uploading h5py wheels
##################################
...
https://travis-ci.org/MacPython/h5py-wheels
...
* builds a h5py wheel, linking against these fresh builds;
...
* making a commit to the ``h5py-wheels`` repository (e.g. with ``git commit
  --allow-empty``); or
...
Which h5py commit does the repository build?
============================================

The ``h5py-wheels`` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t macosx h5py 2.6.0
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 h5py 2.6.0
...
``h5py`` is the root name of the wheel(s) to download / upload, and ``2.6.0``
...
``h5py-2.6.0-`` from http://wheels.scipy.org to ``~/wheelhouse``, then upload
them to PyPI.
