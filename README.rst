...
Repository to build h5py wheels.

To update:

* Update h5py submodule with version you want to build:

    * cd h5py && git pull && git checkout DESIRED_TAG
    * cd .. && git add h5py

* Check minimum numpy versions to build against in ``.travis.yml`` file.  You
  need to build against the earliest numpy that h5py is compatible with; see
