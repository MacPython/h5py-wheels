...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -v -w ~/wheelhouse -t macosx h5py 2.6.0
    wheel-uploader -r warehouse -u $CDN_URL -v -w ~/wheelhouse -t manylinux1 h5py 2.6.0
...
* ``h5py`` is the root name of the wheel(s) to download / upload;
